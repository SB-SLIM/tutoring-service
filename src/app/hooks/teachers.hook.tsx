...
const reducer = (state: State, action: Action) => {
  switch (action.type) {
    ...
    case SORT_TEACHERS:
      ...
      let tmps = [...all_teachers!];

      const sortDin = (property: string) => {
        var sortOrder = 1;
        if (property[0] === "-") {
          sortOrder = -1;
          property = property.substr(1);
        }
        return (a: TTeacher, b: TTeacher) => {
          var result =
      ...
      if(state.sort.speaks !== 'all'){
      return {
        ...state,
        sorted_teachers: tmps.sort(sortDin(state.sort.speaks)),
      };
      }
      if (state.sort.gender !== "all") {
        console.log(state.sort.gender);
        return {
          ...state,
          sorted_teachers: tmps.sort(sortDin('gender')),
        };

      }
      if (state.sort.bestMatch !== "best match") {
         return {
           ...state,
           sorted_teachers: tmps.sort(sortDin(`-${state.sort.bestMatch}`)),
         };
      }else{
